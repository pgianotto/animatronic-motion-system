...
/* ── Scrub slider ──────────────────────────────────────────────────────── */
.pc-scrub { width:100%; accent-color:var(--cyan); cursor:pointer; height:4px;
            margin:4px 0 6px; }

/* ── Session info bar ──────────────────────────────────────────────────── */
#sess-info-bar { background:var(--accent); border-radius:5px; padding:6px 10px;
                 font-size:11px; font-family:monospace; color:var(--fg);
                 margin-bottom:8px; display:none; }
#sess-info-bar span { color:var(--muted); margin-right:4px; }

/* ── Camera ────────────────────────────────────────────────────────────── */
.cam-container { position:relative; background:#000; border-radius:4px;
...
/* ── Waveform canvas ───────────────────────────────────────────────────── */
#wf-canvas { display:block; width:100%; cursor:crosshair;
             border-radius:4px; background:var(--dark); }
#wf-canvas.no-session { cursor:default; }
.wf-filter { display:flex; align-items:center; gap:6px; flex-wrap:wrap;
             margin-bottom:10px; }
.wf-filter label { color:var(--fg); font-size:12px; cursor:pointer;
...
#wf-custom-checks label { color:var(--muted); font-size:11px; cursor:pointer;
                           display:flex; align-items:center; gap:3px; }
#wf-custom-checks input { accent-color:var(--cyan); cursor:pointer; }
#wf-msg { color:var(--muted); font-size:12px; text-align:center;
...
.jm-bar-bg { flex:1; height:6px; background:var(--dark); border-radius:3px;
             overflow:hidden; min-width:50px; max-width:90px; }
.jm-bar    { height:100%; background:var(--cyan); border-radius:3px; transition:width .1s; }
.jm-val    { color:var(--cyan); font-family:monospace; font-size:10px;
...
<div style="display:flex; gap:14px; margin-bottom:14px; align-items:flex-start;">
  ...
  <div class="pc-card" style="width:290px; flex-shrink:0;">
    ...
    <div class="rp-section">
      ...
      <!-- Info bar (shows when a session is loaded) -->
      <div id="sess-info-bar"
           style="<?= $session_name ? '' : 'display:none' ?>">
        <span>Session</span><?= htmlspecialchars($session_name) ?><br>
        <span>Frames</span><span id="info-frames"><?= $fc ?></span>
        &nbsp;&nbsp;<span>Duration</span><span id="info-dur"><?= $dur ?></span>
      </div>
      ...
      <div style="display:flex; align-items:center; gap:6px; font-size:11px; font-family:monospace; min-height:18px;">
        <span class="pc-badge <?= $playing ? 'badge-play' : 'badge-idle' ?>" id="badge-play" style="font-size:10px; padding:2px 8px;">
          <?= $playing ? ($paused ? '⏸ PAUSED' : '▶ PLAYING') : 'STOPPED' ?>
        </span>
        <span id="pb-pos" style="color:var(--muted);">—</span>
      </div>
    </div>

    <!-- Session Files -->
    <div class="rp-section">
      <div class="rp-hdr">Session Files</div>
      <div style="display:flex; gap:5px; margin-bottom:6px;">
        <input class="pc-input" id="sess-save-name" value="session.json" style="flex:1; min-width:0; font-size:11px;">
      ...
      <div id="status-msg" class="pc-msg" style="color:var(--green);"></div>
    </div>
...
<div class="pc-card">
  ...
  <p class="pc-hint">
    Exports an FSEQ v2 file directly to <code>/home/fpp/media/sequences/</code>.
  ...
  <div class="pc-field">
    ...
    <input class="pc-input" id="fseq-name" value="capture.fseq" style="width:200px;">
    <button class="pc-btn btn-export" onclick="exportFseq()">Export FSEQ</button>
  </div>
  <div id="export-msg" class="pc-msg"></div>

  <div id="export-xlights" style="display:none; margin-top:10px; padding:10px;
       background:var(--dark); border-radius:5px; border:1px solid #1a2a4a;">
    <div style="color:var(--cyan); font-weight:bold; font-size:11px; margin-bottom:6px;">xLights Channel Mapping</div>
    ...
    <div style="color:#555; font-size:11px; margin-top:6px; padding-top:6px; border-top:1px solid #1a2a4a;">
      In xLights: add a <strong style="color:var(--fg);">Servo</strong> model per port, set it to the channel pair shown above.
    </div>
  </div>

  <hr style="border:none; border-top:1px solid var(--div); margin:14px 0 10px;">
  <div style="color:var(--muted); font-size:10px; font-weight:bold; letter-spacing:1.5px; text-transform:uppercase; margin-bottom:6px;">xLights Sequence Export</div>
  <p class="pc-hint">Exports a paired <code>.xsq</code> + <code>.fseq</code> for xLights. Place both in your sequences folder, then open the <code>.xsq</code>.</p>
  <div class="pc-field">
    <span class="pc-label">Base name</span>
    <input class="pc-input" id="xsq-name" value="capture" style="width:160px;" placeholder="no extension">
...
<div class="pc-card">
  ...
  <p class="pc-hint" style="margin-bottom:10px;">
    Map each joint to a servo port. Enable <strong>Live Output</strong> to drive servos in real time.
  ...
  <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px 32px; max-width:600px;">
    <div>
      <div style="color:var(--muted); font-size:11px; margin-bottom:4px;">Joint Smoothing
        <span style="color:#444; font-size:10px; margin-left:4px;">lower = smoother/slower</span></div>
    ...
    <div>
      <div style="color:var(--muted); font-size:11px; margin-bottom:4px;">Servo Smoothing
        <span style="color:#444; font-size:10px; margin-left:4px;">output µs damping</span></div>
      <div style="display:flex; align-items:center; gap:8px;">
...
// ── Waveform ──────────────────────────────────────────────────────────────
const WF = {
  data:     null,   // { total_frames, duration, timestamps, data, servo_mapped }
  filter:   'all',
  checked:  Object.fromEntries(JOINTS.map(j => [j.key, true])),
  cursor:   0,      // current frame from status poll
  dragging: false,
  ...
  visible() {
    ...
    if (this.filter === 'servo') {
      return JOINTS.filter(j => this.data.servo_mapped.includes(j.key) && keys.includes(j.key));
    }
    if (this.filter === 'select') {
      return JOINTS.filter(j => this.checked[j.key] && keys.includes(j.key));
    }
  ...
  load() {
    fetch(API + '/api/session/frames')
      ...
      .then(d => {
        ...
        this.cursor = 0;
        if (d.total_frames > 0) {
          document.getElementById('wf-canvas').classList.remove('no-session');
          document.getElementById('wf-msg').style.display = 'none';
        } else {
          document.getElementById('wf-canvas').classList.add('no-session');
          document.getElementById('wf-msg').style.display = '';
        }
  ...
  buildCustomChecks() {
    ...
    JOINTS.forEach(j => {
      ...
      cb.type    = 'checkbox';
      cb.checked = this.checked[j.key] !== false;
      cb.id      = 'wf-cb-' + j.key;
  ...
  draw() {
    ...
    ctx.fillRect(0, 0, cssW, this.RULER_H);
    ctx.strokeStyle = '#333';
    ctx.lineWidth = 1;
    ...
    vis.forEach((j, ri) => {
      ...
      ctx.fillStyle = rowBg;
      ctx.fillRect(0, ry, cssW, this.ROW_H);

      if (mapped) {
        ctx.fillStyle = color;
        ctx.fillRect(0, ry, 3, this.ROW_H);
      }
  ...
  drawPlayhead(ctx, cssW, waveX, waveW, dur, vis) {
    const frames = this.data ? this.data.total_frames : 0;
    if (!frames) return;
    // Map cursor frame index to timestamp
    const n     = this.data.timestamps.length;
...
function wfSetFilter(mode) {
  WF.filter = mode;
  const checks = document.getElementById('wf-custom-checks');
  checks.style.display = mode === 'select' ? 'flex' : 'none';
  ...
  // Rebuild checkboxes to reflect state
...
function onFpsChange() {
  ...
  const ci  = document.getElementById('fps-custom-val');
  ci.style.display = val === 'custom' ? '' : 'none';
  updateFpsLabel(val === 'custom' ? (parseInt(ci.value)||50) : parseInt(val));
}
...
function sessLoad() {
  ...
  fetch(API+'/api/session/load', {
    ...
    if (d.ok) {
      ...
      WF.load();            // fetch waveform data and draw
      pbStart();            // auto-start playback from cursor
    } else {
...
function updateSessionInfo(name, frames, dur) {
  const bar = document.getElementById('sess-info-bar');
  bar.style.display = '';
  bar.innerHTML = `<span>Session</span>${name} &nbsp;&nbsp; <span>Frames</span>${frames} &nbsp;&nbsp; <span>Duration</span>${dur}s`;
  const sl = document.getElementById('scrub-slider');
...
function exportFseq() {
  ...
  msg.style.color = '#888'; msg.textContent = 'Exporting…';
  fetch(API+'/api/config', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({step_time_ms:step_ms})});
  fetch(API+'/api/export', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({filename:name, step_time_ms:step_ms})})
    .then(r=>r.json()).then(d => {
      if (d.ok) {
        msg.style.color='#06d6a0';
        msg.textContent=`✓ ${d.frames} frames · ${(1000/step_ms).toFixed(0)} fps · ${d.duration}s · ${d.channels} ch → ${d.path}`;
        showXLightsMap(d);
...
function pollStatus() {
  return fetch(API+'/api/status').then(r=>r.json()).then(s => {
    ...
    document.getElementById('pb-pos').textContent = s.playing
      ? `${String(Math.floor(s.pb_timestamp/60)).padStart(2,'0')}:${(s.pb_timestamp%60).toFixed(1).padStart(4,'0')}  (${s.pb_pos+1}/${s.frame_count})`
      : '—';
    ...
    if (s.session_name) {
      const bar = document.getElementById('sess-info-bar');
      bar.style.display = '';
      document.getElementById('info-frames').textContent = s.frame_count;
      document.getElementById('info-dur').textContent    = s.duration_str;
      const sl = document.getElementById('scrub-slider');
